...
static void imu_mag_read(int16_t magData[3]);
static void imu_gyro_read(int16_t gyroData[3]);
...
static bool imu_mag_test(){
    ...
    printf("\nAK8963 (Magnetometer chip) Whoami: %x",result);
    return result==0x48;
}
...
static void imu_mag_read(int16_t magData[3]){
    ...
    const uint8_t EXT_SENS_DATA_00_ADDR = 0x49;
    //The IMU then puts it in the FIFO and we read it
    mpu9250_read(EXT_SENS_DATA_00_ADDR,magDataRaw,6);
