...
static const float t_p_weight = 30; //makes it go
static const float t_i_overshoot_weight = 8e-6; //Gets it started if we overshot
static const float t_i_undershoot_weight = 2e-6; //Keeps it going so we don't undershoot
static const float t_d_weight = 4e6; //damps it

void turn_robot(float radians){
    ...
    const bool clockwise = radians>0;
    while (1) {
        if(auto_mode && connected){
            ...
            throttle =  t_p_weight*error +
                        t_d_weight*derivative; 
            if(error>0 == radians>0){ //i.e. we're undershooting 
                //Integral causes overshooting on the way there and so isn't very helpful
                //Undershoot weight is typically lower
                if(fabs(integral*t_i_undershoot_weight)>20.0){
                    integral = ((integral>0)?1:-1) * 20.0/t_i_undershoot_weight;
                }
                throttle += t_i_undershoot_weight*integral;
            }
            else{ //i.e. we're overshooting
                //But is useful for correcting an overshoot
                if(fabs(integral*t_i_overshoot_weight)>20.0){
                    integral = ((integral>0)?1:-1) * 20.0/t_i_overshoot_weight;
                }
                throttle += t_i_overshoot_weight*integral;
            }
            printf("\nP: %f IU: %f IO: %f D: %f Throttle: %f",t_p_weight*error,t_i_undershoot_weight*integral,t_i_overshoot_weight*integral,t_d_weight*derivative,throttle);
            if(throttle>50) throttle = 50;
            if(throttle<-50) throttle = -50;
            update_throttle(50+throttle,50-throttle);
...
void turn_robot_sleep(float radians, int sleep_millis){
    sleep_ms(sleep_millis); //Stops the turn and straight merging which causes an unpredictable turn
    turn_robot(radians);
    sleep_ms(sleep_millis); 
}

void turn_robot_safe(float radians){
    turn_robot_sleep(radians,1000);
}
...
float straight(float distance,bool obstacle_avoidance_enabled){ 
    ...
    while(1){
        if(auto_mode && connected){
            ...
            if(us_distance<0.10){ //Also will stop if us_distance = -1
                printf("\nObject is too close");
                update_throttle(50,50);
...
void check_side(bool right,int *side_check_count,bool *side_invalid,bool *object_in_front,movement_stack *ms){
    ...
    if(side_distance>0.3){
        if(*side_check_count==0){
            turn_robot_safe((right?1:-1)*M_PI_2); //turn to the side
        ...
        if(distance_travelled<(0.15-0.03)){
            *side_invalid = true; //if we stopped prematurely, there's an obstacle
            ...
            if(front_distance>0.3){
                //Space in front
                turn_robot_safe((right?-1:1)*M_PI_2);//turn back
    ...
    while(!ms_isEmpty(ms)){
        ...
        switch(mv.type){
            case STRAIGHT:
                ...
                break;
            case TURN:
                turn_robot_safe(-mv.value); //turn the opposite way
                break;
            default:
                break;
        }
